...
begin
  ...
  begin
    ...
    if Reg.ValueExists('LastUsedDirectory') then
			FDir := Reg.ReadString('LastUsedDirectory')
    else
      FDir := '';
  ...
  if not DirectoryExists(FDir) then FDir := '';

  if (AutoOpen) or (SelDir(FDir,False))  then
  begin
    AutoOpen := False;
    Log('i',lwLngTrns(name,['Directory % selected',FDir]));
    if not FileExists(FDir + '\lamedb') then
			SetVersionDefaults(2)
    else
    	SetVersionDefaults(3);

    if not FileExists(FDir + '\' + ServFilename) then
		begin
    ...
    if (not FileExists(FDir + '\bouquets')) and
       (not FileExists(FDir + '\lamedb')) then
		begin
      MessageDlg(lwLngTrns(name,[
    ...
    begin
      if tv.Items.Item[i].Level = 1 then
		  	dispose(tv.Items.Item[i].Data);
    ...
    if SettingsVersion = 2 then
			sb.Panels[2].Text := lwLngTrns(name,['Satellite'])
    else
   		sb.Panels[2].Text := '';
    ...
    if SettingsVersion <> 2 then
      ...
      if FileExists(FDir+'\settings') then
		  begin
        dbset := TStringList.Create;
        dbset.LoadFromFile(FDir+'/settings');

        if pos('whitelist',lowercase(dbset.Text)) > 0 then
    		begin
          Blacklist := False;
          lfn := 'whitelist';
    ...
    if FileExists(FDir+'\'+lfn) then
      ...
      begin
        if pos('parental',lowercase(sl[i])) > 0 then
        	continue;

        if trim(sl[i]) <> '' then
    		begin
          ParseString(sl[i],psl);
          if not CheckStringsOK(FDir + '\' + lfn, sl[i], psl, 7, 'NNHHHHH') then
    			begin
            Inc(ServErr);
            continue;
          ...
          if psl.Strings[10] = '' then
    			begin
            cdsLock.FieldByName('lockServType').AsInteger :=
             StrToInt('$0'+psl.Strings[2]);

            cdsLock.FieldByName('lockSID').AsInteger :=
             StrToInt('$0'+psl.Strings[3]);

            cdsLock.FieldByName('lockTSID').AsInteger :=
             StrToInt('$0'+psl.Strings[4]);

            cdsLock.FieldByName('lockNID').AsInteger :=
             StrToInt('$0'+psl.Strings[5]);

            cdsLock.FieldByName('lockUniq').AsInteger :=
             StrToInt('$0'+psl.Strings[6]);
            ...
            cdsLock.FieldByName('lockString').AsString := '';
			    ...
			    begin
            if psl.Count >= 11 then
    				begin
              cdsLock.FieldByName('lockString').AsString := psl.Strings[10];

              psl.Strings[10] := StringReplace(psl.Strings[10], '/', '\',
               [rfReplaceAll]);

              cdsLock.FieldByName('lockFilename').AsString :=
               ExtractFilename(psl.Strings[10]);
      ...
      begin
        if (cdsLock.FieldByName('lockI2').AsInteger = 0) and
           (cdsServ.FindKey([cdsLock.FieldByName('lockSID').AsString,
                             ...
                             cdsLock.FieldByName('lockServType').AsString])) then
    		begin
          cdsServ.Edit;
    ...
    if FileExists(FDir+'\bouquets') then
		begin
      ...
      begin
        ...
        if cdsLock.Locate('lockServType;lockSID;lockTSID',
         VarArrayOf(['-3','18',StrToInt(bnr)]),[]) { -3 = fffffffd } then
    		begin
        ...
        while (s <> '/') and (not eof(tf)) do
        begin
          add := true;
          ParseString(s,psl);

          if not CheckStringsOK(FDir+'\bouquets',s,psl,5,'HHHHH') then
    			begin
            Inc(BouqErr);
            add := False;
          ...
          if add then
            if not StrIsNumeric(psl.Strings[4],False) then
            	psl.Strings[4] := IntToStr(StrToInt('$'+psl.Strings[4]));

          if trim(s) = '' then add := false;

          if (add) and (psl.Count < 5) then
    			begin
            MessageDlg(lwLngTrns(name,
             ['Invalid line found (not enough values) and skipped in file ' +
              '"bouquet"']), mtWarning,[mbOK],0);

            log('w', lwLngTrns(name, ['Invalid line found (not enough ' +
             'values) and skipped in file "bouquet"']));
          ...
          if add then
            ...
            cdsFBQ.FieldByName('fbqName').AsString := CleanText(
             cdsServ.FieldByName('servName').AsString);

            cdsFBQ.FieldByName('fbqSID').AsString := IntToStr(
             StrToInt('$0' + psl.Strings[0]));

            cdsFBQ.FieldByName('fbqUniq').AsString := IntToStr(
             StrToInt('$0' + psl.Strings[1]));

            cdsFBQ.FieldByName('fbqTSID').AsString := IntToStr(
             StrToInt('$0' + psl.Strings[2]));

            cdsFBQ.FieldByName('fbqNID').AsString := IntToStr(
             StrToInt('$0' + psl.Strings[3]));
            ...
            cdsFBQ.FieldByName('fbqDbeNr').AsString :=
             cdsServ.FieldByName('servDbeNr').AsString;
    ...
    if FileExists(FDir + '\' + EplTVFilename) then
		begin
      ...
      if SettingsVersion = 2 then
        ...
        begin
          ...
          if (LowerCase(LeftStr(s,8)) = '#service') and
             (LowerCase(RightStr(s,3)) = '.tv') then
			    begin
        ...
        begin
          ...
          if LowerCase(LeftStr(s,8)) = '#service' then
    			begin
            for i := length(s) downto 0 do
            begin
              if s[i] = ':' then
      ...
      begin
        ...
        if (SettingsVersion <> 2) and (pos('"',fsl[i]) > 0) then
    		begin
          ib := pos('"',fsl[i]);
          ie := posex('"',fsl[i],ib+1);
        ...
        if FileExists(FDir+'\' + fsl[i]) then
          ...
          if cdsLock.Locate('lockFilename',VarArrayOf([fsl[i]]),[]) then
			    begin
            pListData.Locked := True;
            cdsLock.Delete;
          ...
          begin
            ...
            if copy(s,1,8) <> '#SERVICE' then
            	continue;

            ParseString(s,psl);

            if not CheckStringsOK(FDir+'\'+fsl[i],s,psl,8,'SSSHHHHH') then
				    begin
              Inc(FTVErr);
              continue;
            ...
            if (SettingsVersion <> 2) and
              ...
              if FileExists(FDir+'\'+altfn) then
                ...
                if alt.Count >= 2 then
                  ...
                  for a := 1 to 1 {alt.Count - 1} do
                  begin { for now: just the irst entry }
                    cdsAlt.Append;

                    cdsAlt.FieldByName('altBQName').AsString := bn;

                    cdsAlt.FieldByName('altSID').AsString := IntToStr(
                     StrToInt('$0' + psl.Strings[4]));

                    cdsAlt.FieldByName('altUniq').AsString := IntToStr(
                     StrToInt('$0' + psl.Strings[7]));

                    cdsAlt.FieldByName('altTSID').AsString := IntToStr(
                     StrToInt('$0' + psl.Strings[5]));

                    cdsAlt.FieldByName('altNID').AsString := IntToStr(
                     StrToInt('$0' + psl.Strings[6]));

                    cdsAlt.FieldByName('altServType').AsString :=
                     psl.Strings[3];
            ...
            if SettingsVersion <> 2 then
            	psl.Strings[3] := IntToStr(StrToInt('$'+psl.Strings[3]));

            if psl.Strings[1] <> '1' then
    				begin
              Log('w', lwLngTrns(name,
            ...
            { Handle Markers }
            if psl.Strings[2] = '64' then
    				begin
              if (e < sl.Count - 1) and
               (Copy(sl[e + 1], 1, 12) = '#DESCRIPTION') then
              begin
                if copy(sl[e+1],1,13) = '#DESCRIPTION:' then
                	descr := trim(copy(sl[e+1],14,length(sl[e+1])-1))
                else
    							descr := trim(copy(sl[e+1],13,length(sl[e+1])-1))

              end
              else
					    begin
                Log('w',lwLngTrns(name,['Empty marker ignored (%)', fsl[i]]));
            ...
            if (descr = '') and
               ...
               (cdsFTV.FindKey([IntToStr(i),
    				...
    				begin
              Log('w', lwLngTrns(name,
               ['Duplicate TV favourite (%) not loaded', fsl[i]]));
            ...
            if (ubt = 'n') or (ubt = 'r') then
            	cdsFTV.FieldByName('ftvName').AsString := CleanText(
               cdsServ.FieldByName('servName').AsString)
            else
    					cdsFTV.FieldByName('ftvName').AsString := CleanText(descr);

            cdsFTV.FieldByName('ftvSID').AsString := IntToStr(
             StrToInt('$0' + psl.Strings[4]));

            cdsFTV.FieldByName('ftvUniq').AsString := IntToStr(
             StrToInt('$0' + psl.Strings[7]));

            cdsFTV.FieldByName('ftvTSID').AsString := IntToStr(
             StrToInt('$0' + psl.Strings[5]));

            cdsFTV.FieldByName('ftvNID').AsString := IntToStr(
             StrToInt('$0' + psl.Strings[6]));

            if ubt = 'm' then
            	cdsFTV.FieldByName('ftvServType').AsString := '0'
            else
    					cdsFTV.FieldByName('ftvServType').AsString := psl.Strings[3];

            cdsFTV.FieldByName('ftvSetNr').AsInteger := i;

            if descr <> '' then
            	cdsFTV.FieldByName('ftvDescr').AsString := CleanText(descr)
            else
    					cdsFTV.FieldByName('ftvDescr').AsString := '';

            if (ubt = 'n') or (ubt = 'r') then
            	cdsFTV.FieldByName('ftvDbeNr').AsString :=
               cdsServ.FieldByName('servDbeNr').AsString
            else
    					cdsFTV.FieldByName('ftvDbeNr').AsString := '0';
    ...
    if FileExists(FDir + '\' + EplRDFilename) then
		begin
      fsl := TStringList.Create;
      ...
      if SettingsVersion = 2 then
        ...
        begin
          ...
          if (LowerCase(LeftStr(s,8)) = '#service') and
             (LowerCase(RightStr(s,6)) = '.radio') then
    			begin
            for i := length(s) downto 0 do
              if s[i] = '/' then
              	Break;
      ...
      begin
        ...
        if FileExists(FDir + '\' + fsl[i]) then
          ...
          begin
            ...
            if not CheckStringsOK(FDir + '\' + fsl[i], s, psl, 8,
             'SSSHHHHH') then
    				begin
              Inc(FRDErr);
            ...
            if (psl.Strings[2] = '15') and (psl.Strings[3] <> 'fffffffd') then
    				begin
              Log('w', lwLngTrns(name,
               ['Unsupported reference in Radio User Bouquet ' +
								'(%) skipped:',fsl[i]]));
            ...
            if (descr = '') and
               ...
               (cdsFRD.FindKey([IntToStr(i),
    				...
    				begin
              Log('w', lwLngTrns(name,
               ['Duplicate Radio favourite (%) not loaded', fsl[i]]));
            ...
            if (ubt = 'n') or (ubt = 'r') then
            	cdsFRD.FieldByName('frdName').AsString := CleanText(cdsServ.FieldByName('servName').AsString)
            else
    					cdsFRD.FieldByName('frdName').AsString := CleanText(descr);
            ...
            if ubt = 'm' then
            	cdsFRD.FieldByName('frdServType').AsString := '0'
            else
    					cdsFRD.FieldByName('frdServType').AsString := psl.Strings[3];

            cdsFRD.FieldByName('frdSetNr').AsInteger := i;

            if descr <> '' then
            	cdsFRD.FieldByName('frdDescr').AsString := CleanText(descr)
            else
    					cdsFRD.FieldByName('frdDescr').AsString := '';

            if (ubt = 'n') or (ubt = 'r') then
            	cdsFRD.FieldByName('frdDbeNr').AsString := cdsServ.FieldByName('servDbeNr').AsString
            else
    					cdsFRD.FieldByName('frdDbeNr').AsString := '0';
    ...
    if FileExists(FDir+'\satellites.xml') then
		begin
      ...
      begin
        ...
        begin
          ...
          if cdsPos.FindKey([StrToInt(copy(s,ib+10,ie-ib-10))]) then
    			begin
            ib := pos('sat name="',s);
    ...
    if (ServErr = 0) and
       ...
       (FTVErr = 0) and
       (FRDErr = 0) then
			log('i',lwLngTrns(name,['Files loaded']))
    else
    begin
    ...
    if (satcable = 's') and (not FileExists(FDir + '\satellites.xml')) then
		begin
      MessageDlg(lwLngTrns(name,[
      ...
      Log('w',lwLngTrns(name,['File % not found in "%"', 'satellites.xml',FDir]));
...
begin
  ...
  if not FileExists(FDir + '\' + ServFilename + '_org')  then
  begin
    if RenameFile(FDir + '\' + ServFilename,FDir + '\' + ServFilename + '_org') then
  ...
  begin
    if FileExists(FDir + '\' + ServFilename + '_bak') then
		begin
    ...
    if RenameFile(FDir + '\' + ServFilename,FDir + '\' + ServFilename + '_bak') then
			log('i',lwLngTrns(name,['% renamed to % in %',
  ...
  if SettingsVersion = 2 then
  	lfn := 'services.locked'
  else if Blacklist then
		lfn := 'blacklist'
  else
    lfn := 'whitelist';

  if not FileExists(FDir + '\' + lfn + '_org')  then
  begin
    if RenameFile(FDir + '\' + lfn,FDir + '\' + lfn + '_org') then
			Log('i',lwLngTrns(name,['% renamed to % in %',
        lfn,lfn + '_org',FDir]));
  ...
  begin
    if FileExists(FDir + '\' + lfn + '_bak') then
		begin
    ...
    if RenameFile(FDir + '\' + lfn,FDir + '\' + lfn + '_bak') then
			log('i',lwLngTrns(name,['% renamed to % in %',
        lfn,lfn + '_bak',FDir]));

  end;

  if not FileExists(FDir + '\bouquets_org')  then
  begin
    if RenameFile(FDir + '\bouquets',FDir + '\bouquets_org') then
			Log('i',lwLngTrns(name,['% renamed to % in %',
  ...
  begin
    if FileExists(FDir + '\bouquets_bak') then
		begin
    ...
    if RenameFile(FDir + '\bouquets',FDir + '\bouquets_bak') then
			log('i',lwLngTrns(name,['% renamed to % in %',
  ...
  if SettingsVersion = 2 then
  	WriteLn(tf,'eDVB services /2/');

  if SettingsVersion = 3 then
  	WriteLn(tf,'eDVB services /3/');

  if SettingsVersion = 4 then
  	WriteLn(tf,'eDVB services /4/');
  ...
  begin
    ...
    begin
      if cdsTSID.FieldByName('tsidSTC').AsString = 's' then
  			s := #09 + 's ' +
                cdsTSID.FieldByName('tsidFreq').AsString + ':' +
                cdsTSID.FieldByName('tsidSymb').AsString + ':' +
                cdsTSID.FieldByName('tsidPol').AsString + ':' +
  ...
  begin
    ...
    if cdsServSave.FieldByName('servNr').AsString <> '' then
			s := s + ':' + cdsServSave.FieldByName('servNr').AsString
    else
    	s := s + ':' + IntToStr(c+200);
    ...
    if cdsServSave.FieldByName('servC08PCMDelay').AsInteger <> 0 then
			s := 'c:08' + IntToHex(StrToInt(cdsServSave.FieldByName('servC08PCMDelay').AsString),4) + ',' + s;
			
    if cdsServSave.FieldByName('servC07BSDelay').AsInteger <> 0 then
			s := 'c:07' + IntToHex(StrToInt(cdsServSave.FieldByName('servC07BSDelay').AsString),4) + ',' + s;

    if cdsServSave.FieldByName('serv3PID').AsInteger <> 0 then
			s := 'c:04' + IntToHex(StrToInt(cdsServSave.FieldByName('serv3PID').AsString),4) + ',' + s;

    if cdsServSave.FieldByName('servPPID').AsInteger <> 0 then
			s := 'c:03' + IntToHex(StrToInt(cdsServSave.FieldByName('servPPID').AsString),4) + ',' + s;

    if cdsServSave.FieldByName('servTPID').AsInteger <> 0 then
			s := 'c:02' + IntToHex(StrToInt(cdsServSave.FieldByName('servTPID').AsString),4) + ',' + s;

    if cdsServSave.FieldByName('servAPID').AsInteger <> 0 then
			s := 'c:01' + IntToHex(StrToInt(cdsServSave.FieldByName('servAPID').AsString),4) + ',' + s;

    if cdsServSave.FieldByName('servVPID').AsInteger <> 0 then
			s := 'c:00' + IntToHex(StrToInt(cdsServSave.FieldByName('servVPID').AsString),4) + ',' + s;

    if cdsServSave.FieldByName('servFlags').AsInteger  and 127 <> 0 then
			s := 'f:' + IntToHex(cdsServSave.FieldByName('servFlags').AsInteger and 127,4) + ',' + s;

    { Only for version 4  and higher }
    if (SettingsVersion >= 4) and
       (cdsServSave.FieldByName('servExtra').AsString <> '') then
			s := s + ',' + cdsServSave.FieldByName('servExtra').AsString; { Append unknown exra fields }
  ...
  if SettingsVersion = 2 then
  	WriteLn(tf,'Parentallocked Services');
  ...
  while not cdsServSave.Eof do
  begin
    if cdsServSave.FieldByName('servLock').AsInteger = 1 then
		begin
      s := '1:0:' +
  ...
  for i := 0 to tv.Items.Count - 1 do
  begin
    if (tv.Items[i].Data = nil) or (tv.Items[i].Level = 0) then
			continue;

    pListData := tv.Items.Item[i].Data;

    if not pListData.Locked then
			continue;

    if tv.Items[i].Parent.Text[1] = 'P' then
		begin
      s := '1:15:fffffffd:12:' +
  ...
  begin
    ...
    begin
      if (tv.Items.Item[i].Level <> 1) or
         (tv.Items.Item[i].Parent.Text[1] <> 'P') then
  			continue;
      ...
      while (not cdsFBQ.Eof) and
            (cdsFBQ.FieldByName('fbqSetNr').AsInteger = pListData.Number) do
			begin;
        //Inc(p);
        //  if p mod 25 = 0
        //  then ShowWait('pos',0,0,p);
  ...
  begin
    if (tv.Items.Item[i].Level <> 1) or
       (tv.Items.Item[i].Parent.Text[1] <> 'T') then
			continue;
    ...
    if FileExists(FDir + '\' + fn) then
		begin
      if not FileExists(FDir + '\' + fn + '_org') then
  		begin
        if RenameFile(FDir + '\' + fn, FDir + '\' + fn + '_org') then
        	Log('i',lwLngTrns(name,['% renamed to % in %', fn,fn + '_org', FDir]));
      end
      else
    	begin
        if FileExists(FDir + '\' + fn + '_bak') then
    		begin
          DeleteFile(FDir + '\' + fn + '_bak');

          Log('i',lwLngTrns(name,['% deleted from %', fn + '_bak', FDir]));

        end;

        if RenameFile(FDir + '\' + fn, FDir + '\' + fn + '_bak') then
        	log('i',lwLngTrns(name,['% renamed to % in %', fn,fn + '_bak', FDir]));
    ...
    begin
      ...
      if (SettingsVersion <> 2) and
         (cdsAlt.FindKey([tv.Items.Item[i].Text,
                          ...
                          cdsFTV.FieldByName('ftvServType').AsString])) then
  			s := '#SERVICE 1:134:1:0:0:0:0:0:0:0:FROM BOUQUET "' +
        ...
        if SettingsVersion = 2 then
        	stype := cdsFTV.FieldByName('ftvServType').AsString
        else
    			stype := LowerCase(IntToHex(StrToInt(cdsFTV.FieldByName('ftvServType').AsString),1));

        if cdsFTV.FieldByName('ftvT').AsString <> 'm' then
        	s := '#SERVICE' + v2Sep + ' 1:0:' +
                  stype + ':' +
      ...
      if cdsFTV.FieldByName('ftvDescr').AsString <> '' then
		  begin
    ...
    Log('i',lwLngTrns(name,['TV User Bouquet % saved in %', tv.Items.Item[i].Text, FDir]));

  end;

  if FileExists(FDir + '\' + EplTVFilename)  then
  begin
    if not FileExists(FDir + '\' + EplTVFilename + '_org') then
		begin
      if RenameFile(FDir + '\' + EplTVFilename, FDir + '\' + EplTVFilename + '_org') then
  			Log('i',lwLngTrns(name,['% renamed to % in %',
    ...
    begin
      if FileExists(FDir + '\' + EplTVFilename + '_bak') then
  		begin
      ...
      if RenameFile(FDir + '\' + EplTVFilename, FDir + '\' + EplTVFilename + '_bak') then
			  log('i',lwLngTrns(name,['% renamed to % in %',
  ...
  begin
    if (tv.Items.Item[i].Level <> 1) or
       (tv.Items.Item[i].Parent.Text[1] <> 'R') then
			continue;
    ...
    begin
      ...
      if (SettingsVersion <> 2) and
         (cdsAlt.FindKey([tv.Items.Item[i].Text,
                          ...
                          cdsFRD.FieldByName('frdServType').AsString])) then
  			s := '#SERVICE 1:134:1:0:0:0:0:0:0:0:FROM BOUQUET "' +
          cdsAlt.FieldByName('altFileName').AsString +
          '" ORDER BY bouquet'
        ...
        if SettingsVersion = 2 then
        	stype := cdsFRD.FieldByName('frdServType').AsString
        else
    			stype := LowerCase(IntToHex(StrToInt(cdsFRD.FieldByName('frdServType').AsString),1));

        if cdsFRD.FieldByName('frdT').AsString <> 'm' then
        	s := '#SERVICE' + v2Sep + ' 1:0:' +
            stype + ':' +
      ...
      if cdsFRD.FieldByName('frdDescr').AsString <> '' then
		  begin
  ...
  if FileExists(FDir + '\' + EplRDFilename)  then
  begin
    if not FileExists(FDir + '\' + EplRDFilename + '_org') then
		begin
      if RenameFile(FDir + '\' + EplRDFilename, FDir + '\' + EplRDFilename + '_org') then
  			Log('i',lwLngTrns(name,['% renamed to % in %',
    ...
    begin
      if FileExists(FDir + '\' + EplRDFilename + '_bak') then
  		begin
      ...
      if RenameFile(FDir + '\' + EplRDFilename, FDir + '\' + EplRDFilename + '_bak') then
			  log('i',lwLngTrns(name,['% renamed to % in %',
...
begin
  ...
  begin
    ...
    if FMsgTable.IndexOfName(key) >= 0 then
			s := FMsgTable.Values[key];
...
function TfrmMain.ReadServices(const filename: string;
  ...
  begin
    ...
    begin
      ...
      if ie - ib - length(parm) <> lvalue then            // If length is wrong, search again 
  			ib := posex(lowercase(parm),s,ie+1)
      else
    		Break;                      // Found

    end;

    if ib = 0 then                         // Not found 
			exit;
    ...
    if RightStr(line,1) = ',' then
			line := Leftstr(line,length(line)-1);    // Remove trailing ',' if needed
  end;
...
begin
  ...
  begin
    ...
    begin
      if (c = 0) and (tsl[i] = 'transponders') then
  			c := 1
      else
        if (c = 1) and (tsl[i] = 'end') then
        	c := 2
        else if (c = 2) and (tsl[i] = 'services') then
        	c := 3
        else if (c = 3) and (tsl[i] = 'end') then
        	c := 4;
  ...
  if not compare then
    if pos('/2/',s) <> 0 then
			SetVersionDefaults(2)
    else if pos('/3/',s) <> 0 then
  		SetVersionDefaults(3)
    else
    	SetVersionDefaults(4);
  ...
  begin
    ...
    if not CheckStringsOK(filename + ' - Transponders',s,psl,3,'HHH') then
		begin
      Inc(ServErr);
    ...
    if not CheckStringsOK(filename + ' - Transponders',s,psl,6,'SNNNNN') then
		begin
      Inc(ServErr);
    ...
    if (cdsTSID.FieldByName('tsidSTC').AsString = 's') and
       (not cdsPos.FindKey([StrToInt(psl.Strings[5])])) then
		begin
      cdsPos.Append;

      cdsPos.FieldByName('PosInt').AsString := psl.Strings[5];
      cdsPos.FieldByName('PosStr').AsString := Format('%.1n',
        [StrToInt(psl.Strings[5])/10]);

      cdsPos.FieldByName('PosName').AsString :=
        cdsPos.FieldByName('PosStr').AsString;

      if (SettingsVersion = 2) and
         (LowerCase(psl.Strings[0]) = 'c') then
  ...
  begin
    ...
    if not CheckStringsOK(filename + ' - Services',s,psl,6,'HHHHNN') then
		begin
      Inc(ServErr);
    ...
    if cdsTSID.FindKey([IntToStr(StrToInt('$'+psl.Strings[1])),
                        IntToStr(StrToInt('$'+psl.Strings[2])),
                        IntToStr(StrToInt('$'+psl.Strings[3]))]) then
		begin
      cds.Append;
      ...
      cds.FieldByName('servSid').AsString := IntToStr(
        StrToInt('$'+psl.Strings[0]));

      cds.FieldByName('servUniq').AsString := IntToStr(
        StrToInt('$'+psl.Strings[1]));

      cds.FieldByName('servTsid').AsString := IntToStr(
        StrToInt('$'+psl.Strings[2]));

      cds.FieldByName('servNid').AsString := IntToStr(
        StrToInt('$'+psl.Strings[3]));

      cds.FieldByName('servType').AsString := psl.Strings[4];

      cds.FieldByName('servTypeTxt').AsString := TrnsServType(
        cds.FieldByName('servType').AsInteger);

      cds.FieldByName('servNr').AsString := psl.Strings[5];

      if (not Compare) and (psl.Strings[5] <> '') then
  			MaxNr := Max(StrToInt(psl.Strings[5]),MaxNr);

      ReadLn(tf,s);

      cds.FieldByName('servName').AsString := TrimRight(Copy(s, 1, 50));
      cds.FieldByName('servCleanName').AsString := Cleantext(
        TrimRight(copy(s,1,50)));

      ReadLn(tf,s);

      if pos('p:',s) = 0 then// No p: parameter, so whole string is package name
		  begin
        l := length(s);

        if l > 50 then // Copy max 50 characters
          l := 50;
        ...
        if prm <> '' then
        	cds.FieldByName('servVPID').AsString := IntToStr(StrToInt('$'+prm));

        prm := ExtractParm(s,'c:01',4);

        if prm <> '' then
        	cds.FieldByName('servAPID').AsString := IntToStr(StrToInt('$'+prm));

        prm := ExtractParm(s,'c:02',4);

        if prm <> '' then
        	cds.FieldByName('servTPID').AsString := IntToStr(StrToInt('$'+prm));

        prm := ExtractParm(s,'c:03',4);

        if prm <> '' then
        	cds.FieldByName('servPPID').AsString := IntToStr(StrToInt('$'+prm));

        prm := ExtractParm(s,'c:04',4);

        if prm <> '' then
        	cds.FieldByName('serv3PID').AsString := IntToStr(StrToInt('$'+prm));

        prm := ExtractParm(s,'c:07',4);

        if prm <> '' then
        	cds.FieldByName('servC07BSDelay').AsString := IntToStr(
            StrToInt('$'+prm));

        prm := ExtractParm(s,'c:08',4);

        if prm <> '' then
        	cds.FieldByName('servC08PCMDelay').AsString := IntToStr(
            StrToInt('$' + prm));
        ...
        if prm <> '' then
        	cds.FieldByName('servFlags').AsString := IntToStr(StrToInt('$' + prm))
        else
    		begin
          prm := ExtractParm(s, 'f:', 4); { or 4 bytes flags field }

          if prm <> '' then
          	cds.FieldByName('servFlags').AsString := IntToStr(
              StrToInt('$' + prm));
        ...
        ie := min(posex(', c:',lowercase(s)+', c:',ib+1),
          posex(',f:', lowercase(s) + ',f:', ib + 1));
      ...
      cds.FieldByName('servSTC').AsString :=
        cdsTSID.FieldByName('tsidSTC').AsString;

      cds.FieldByName('servFreq').AsString :=
        cdsTSID.FieldByName('tsidFreq').AsString;

      cds.FieldByName('servSymb').AsString :=
        cdsTSID.FieldByName('tsidSymb').AsString;

      cds.FieldByName('servPol').AsString :=
        cdsTSID.FieldByName('tsidPol').AsString;

      cds.FieldByName('servFEC').AsString :=
        cdsTSID.FieldByName('tsidFEC').AsString;

      cds.FieldByName('servTrExtra').AsString :=
        cdsTSID.FieldByName('tsidExtra').AsString;

      if cdsTSID.FieldByName('tsidSTC').AsString = 's' then
			  cds.FieldByName('servPos').AsString :=
          cdsTSID.FieldByName('tsidPos').AsString
      else
    	begin
        if cdsTSID.FieldByName('tsidSTC').AsString = 't' then
    		begin
          cds.FieldByName('servPos').AsString := '8888';

          if not cdsPos.Locate('PosInt',VarArrayOf([8888]),[]) then
    			begin
            cdsPos.Append;
          ...
          if not cdsPos.Locate('PosInt',VarArrayOf([9999]),[]) then
    			begin
            cdsPos.Append;
        ...
        cds.FieldByName('servPos').AsString :=
          cdsPos.FieldByName('PosInt').AsString;
    ...
    begin;
      log('w',
        lwLngTrns(name,['Service without corresponding transponder definition '
         + 'not loaded']));
...
begin
  ...
  begin
    ...
    if (ch = ' ') or (ch = ':') or (p = l) then
      ...
      if (psl.Count = 10) and (LeftStr(s,1) = '/') then
  		begin
        psl.Add(s);
...
begin
  ...
  begin
    if Format[I + 1] = 'S' then
		begin
      if Trim(psl[I]) = '' then
  		begin
        Log('e', lwLngTrns(name,
         ['Invalid line in settings found and skipped: ' +
            'missing parameter %',
            IntToStr(I + 1)]));
	    ...
	    if Format[I+1] = 'H' then
			begin
      	if (Trim(psl[I]) = '') or (not StrIsHexadecimal(psl[I])) then
  			begin
        	Log('e', lwLngTrns(name,
           ['Invalid line in settings found and skipped: '
    ...
    else if Format[I + 1] = 'N' then
		begin
      if (trim(psl[I]) = '') or (not StrIsNumeric(psl[I], False)) then
  		begin
        Log('e', lwLngTrns(name, ['Invalid line in settings found and skipped: '
...
begin
  ...
  while p^ <> #0 do
  begin
    if ((DecPointAllowed) and (not (p^ in ['0'..'9','-','.',',']))) or
     ((not DecPointAllowed) and (not (p^ in ['0'..'9','-']))) then
			Exit;
...
begin
  ...
  if cbTVRadio.ItemIndex = 0 then
  	ft := ''
  else if cbTVRadio.ItemIndex = 1 then
	  ft := 'servTypeTxt = ''TV'''
  else if cbTVRadio.ItemIndex = 2 then
  	ft := 'servTypeTxt = ''Radio'''
  else if cbTVRadio.ItemIndex = 3 then
  	ft := '(servTypeTxt = ''TV'' or servTypeTxt = ''Radio'')'
  else
    ft := '(servTypeTxt <> ''TV'' and servTypeTxt <> ''Radio'')';

  if cbSatName.ItemIndex <= 0 then
  	fs := ''
  else
  begin
    cdsPos.First;
    ...
    begin
      if cdsPos.FieldValues['posName'] =
       cbSatname.Items[cbSatName.ItemIndex] then
  			Break;
    ...
    if not cdsPos.EOF then
			fs := 'servPos = ''' + cdsPos.FieldByName('posInt').AsString + ''''

    else
	    fs := 'servPos = ''' +  cbSatName.Items[cbSatName.ItemIndex] + '''';

  end;

  if trim(ePackage.Text) = '' then
	  ePackage.Text := trim(ePackage.Text);

  if ePackage.Text = '' then
	  fp := ''
  else
    fp := 'servPackage = ''' +  ePackage.Text + '*''';
  ...
  if it <> -1  then
  begin
    if cdsServ.Locate('servDbeNr',VarArrayOf([it]),[]) then
		begin
      it := cdsServ.RecNo - 1;
...
begin
  ...
  begin
    ...
    MessageDlg('Internal error: invalid option passed for menu settings',
      mtError,[mbOK],0);
...
begin
  ...
  if (DetType = 'P') and (DetSetNr >= 0) and (DetChanged)  then
  begin
    ld := NewBouquet('P');
  
    for i := 0 to tnBQ.Count do
    begin
      pListData := tnBQ.Item[i].Data;

      if pListData.Number = DetSetNr then
		  begin
  ...
  for i := 0 to lvDet.Items.Count - 1 do
  begin
    wcds.Append;

    wcds.FieldByName(pf+'SeqNr').AsInteger := i;

    if lvDet.Items[i].SubItems[11] <> 'r' then
			wcds.FieldByName(pf+'Name').AsString := lvDet.Items[i].Caption
    else
    	wcds.FieldByName(pf+'Name').AsString := lvDet.Items[i].SubItems[8];
    ...
    if lvDet.Items[i].SubItems[11] <> 'r' then
			wcds.FieldByName(pf+'Descr').AsString := lvDet.Items[i].SubItems[8]
    else
    	wcds.FieldByName(pf+'Descr').AsString := lvDet.Items[i].Caption;
...
begin
  ...
  if not MultiLang.SetLanguage(Self,Language,False)  then
  begin
    Log('w', 'Error initializing language to '
     + Language + ', defaulted to original English version');

    MessageDlg('Error initializing language to '
     + Language + ', defaulted to original English version',
      mtWarning,[mbOK],0);
  ...
  begin
    if FileExists(ExtractFilePath(application.exename) + Language + '.lng') then
			FMsgTable.LoadFromFile(ExtractFilePath(application.exename)
       + Language + '.lng');
...
begin
  ...
  for i := 0 to lvServ.Columns.Count - 1 do
  begin
  ...
  for i := 0 to lvDet.Columns.Count - 1 do
  begin
...
begin
  ...
  if (tnTV = nil) or (tnRD = nil) or (tnBQ = nil) then
  	_lBouqT.Caption := '0'
  else
    _lBouqT.Caption := IntToStr(tnTV.Count +
                                tnRD.Count +
                                tnBQ.Count);
...
begin
  ...
  begin
    ...
    for nr := 900608 to 1048575 do { is hex DBE00 to FFFFF }
    begin
      ...
      begin
        if (tv.Items.Item[I].Level = 1) and
           (tv.Items.Item[I].Parent.Text[1] = bt) then
    		begin
          pListData := tv.Items.Item[I].Data;

          if pListData.FileName = s then
    			begin
            fnd := True;
    ...
    if nr > 1048575 then
		begin
      MessageDlg(lwLngTrns(name,
        ['Maximum number of 147.967 % user bouquets reached.', ets]),
         mtError, [mbOK], 0);
  ...
  begin
    ...
    for I := -1 downto -999 do
      if bsl.IndexOf(IntToStr(I)) = -1 then
  			Break;
    ...
    if I < -999 then
		begin
      MessageDlg(lwLngTrns(name,
       ['No new bouquet possible. Maximum number of 1000 bouquets is ' +
        'reached.']), mtError,[mbOK],0);
...
begin
  ...
  while (minl <= 35) and (totl <= ToolBar1.Width - 10) do
  begin
    totl := 0;
    ...
    begin
      if (ToolBar1.Buttons[i].Caption <> '') and
         (ToolBar1.Buttons[i].Caption <> 'FTP') then
  			minl := min(minl,ToolBar1.Canvas.TextWidth(ToolBar1.Buttons[i].Caption));

      if minl = ToolBar1.Canvas.TextWidth(ToolBar1.Buttons[i].Caption) then
			  n := i;
    ...
    if (minl <= 35) and (totl <= ToolBar1.Width - 10) then
			ToolBar1.Buttons[n].Caption := ' ' + ToolBar1.Buttons[n].Caption + ' ';
